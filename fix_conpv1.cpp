...
FixConpV1::FixConpV1(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),coulpair(NULL),vlstr(NULL),vrstr(NULL)
{
  if (narg < 11) error->all(FLERR,"Illegal fix conp command");
  vlstyle = vrstyle = CONSTANT;
  ilevel_respa = 0;
  maxiter = 100;
  tolerance = 0.000001;
...
void FixConpV1::setup(int vflag)
{
  //Pair *coulpair;
  ...
  if (coulpair == NULL) error->all(FLERR,"Must use conp4 with coul pair style");
...
//copied from ewald.cpp end
  ...
  memory->create3d_offset(cs,-kmax,kmax,3,nmax,"fixconpv1:cs");
  memory->create3d_offset(sn,-kmax,kmax,3,nmax,"fixconpv1:sn");
...
void FixConpV1::pre_force(int vflag)
{
...
void FixConpV1::pre_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
int FixConpV1::electrode_check(int atomid)
{
...
void FixConpV1::b_cal()
{
  ...
  double evscale = 0.069447;
  if (vlstyle == EQUAL) vL = evscale*(input->variable->compute_equal(vlvar));
  if (vrstyle == EQUAL) vR = evscale*(input->variable->compute_equal(vrvar));
...
void FixConpV1::a_cal()
{
  ...
  double **eleallx = NULL;
  memory->create(eleallx,elenum_all,3,"fixconpv1:eleallx");
  ...
  memory->create(csk,kcount,elenum_all,"fixconpv1:csk");
  memory->create(snk,kcount,elenum_all,"fixconpv1:snk");
  sincos_a(eleallx);
  delete [] elexyzlist;
...
void FixConpV1::sincos_a(double **eleallx)
{
  int i,m,k,ic;
  int kx,ky,kz;
  double ***csele,***snele;
  memory->create3d_offset(csele,-kmax,kmax,3,elenum_all,"fixconpv1:csele");
  memory->create3d_offset(snele,-kmax,kmax,3,elenum_all,"fixconpv1:snele");
...
/* ---------------------------------------------------------------------- */
void FixConpV1::force_cal(int vflag)
{
  int i;
...
/* ---------------------------------------------------------------------- */
void FixConpV1::coul_cal(int coulcalflag,double *m,int *ele2tag)
{
  Ctime1 = MPI_Wtime();
...
/* ---------------------------------------------------------------------- */
double FixConpV1::rms(int km, double prd, bigint natoms, double q2)
{
