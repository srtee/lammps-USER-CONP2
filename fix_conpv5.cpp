...
FixConpV5::FixConpV5(LAMMPS *lmp, int narg, char **arg) :
...
void FixConpV5::request_smartlist() {
...
void FixConpV5::init_list(int /* id */, NeighList *ptr) {
  if (smartlist) {
...
void FixConpV5::setup(int vflag)
{
...
void FixConpV5::pre_force(int /* vflag */)
{
...
void FixConpV5::post_force(int vflag) {
  force_cal(vflag);
}

/* ---------------------------------------------------------------------- */

int FixConpV5::electrode_check(int atomid)
{
...
void FixConpV5::b_comm(double* bsend, double* brecv)
{
...
void FixConpV5::update_bk(bool coulyes, double* bbb_all)
{
  Ktime1 = MPI_Wtime();
  int i,j,k,elei;
  int nmax = atom->nmax;
...
void FixConpV5::sincos_a(int ff_flag,double *eleallz)
{
...
/* ---------------------------------------------------------------------- */
void FixConpV5::force_cal(int vflag)
{
  int i;
...
/*
/* ---------------------------------------------------------------------- */
void FixConpV5::coul_cal(int coulcalflag, double* m)
{
  Ctime1 = MPI_Wtime();
...
/* ---------------------------------------------------------------------- */
void FixConpV5::alist_coul_cal(double* m)
{
  Ctime1 = MPI_Wtime();
  if (alist->occasional) {
    neighbor->build(0);
...
/* ---------------------------------------------------------------------- */
void FixConpV5::blist_coul_cal(double* m)
{
  Ctime1 = MPI_Wtime();
  int i,j,k,ii,jj,jnum,itype,jtype,idx1d;
  int eci,ecj;
...
/* ---------------------------------------------------------------------- */
void FixConpV5::blist_coul_cal_post_force()
{
  Ctime1 = MPI_Wtime();
  int i,j,k,ii,jj,jnum,itype,jtype,idx1d;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz;
...
double FixConpV5::compute_scalar()
{
  return addv;
}
...
double FixConpV5::compute_vector(int n)
{
...
double FixConpV5::rms(int km, double prd, bigint natoms, double q2)
{
