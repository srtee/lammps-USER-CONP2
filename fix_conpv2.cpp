...
FixConpV2::FixConpV2(LAMMPS *lmp, int narg, char **arg) :
...
void FixConpV2::setup(int vflag)
{
  //Pair *coulpair;
...
//copied from ewald.cpp end
  ...
  memory->create3d_offset(cs,-kmax,kmax,3,nmax,"fixconpv2:cs");
  memory->create3d_offset(sn,-kmax,kmax,3,nmax,"fixconpv2:sn");
...
void FixConpV2::pre_force(int vflag)
{
...
void FixConpV2::pre_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
int FixConpV2::electrode_check(int atomid)
{
...
void FixConpV2::a_cal()
{
  ...
  double **eleallx = NULL;
  memory->create(eleallx,elenum_all,3,"fixconpv2:eleallx");
  ...
  memory->create(csk,kcount,elenum_all,"fixconpv2:csk");
  memory->create(snk,kcount,elenum_all,"fixconpv2:snk");
  sincos_a(eleallx);
  delete [] elexyzlist;
...
void FixConpV2::sincos_a(double **eleallx)
{
  int i,m,k,ic;
  int kx,ky,kz;
  double ***csele,***snele;
  memory->create3d_offset(csele,-kmax,kmax,3,elenum_all,"fixconpv2:csele");
  memory->create3d_offset(snele,-kmax,kmax,3,elenum_all,"fixconpv2:snele");
...
/* ---------------------------------------------------------------------- */
void FixConpV2::force_cal(int vflag)
{
  int i;
...
/* ---------------------------------------------------------------------- */
void FixConpV2::coul_cal(int coulcalflag,double *m,int *ele2tag)
{
  Ctime1 = MPI_Wtime();
...
/* ---------------------------------------------------------------------- */
double FixConpV2::compute_scalar()
{
  return addv;
}
...
/* ---------------------------------------------------------------------- */
double FixConpV2::rms(int km, double prd, bigint natoms, double q2)
{
