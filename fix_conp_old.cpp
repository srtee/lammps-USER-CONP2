...
FixConpOld::FixConpOld(LAMMPS *lmp, int narg, char **arg) :
...
void FixConpOld::request_smartlist() {
...
void FixConpOld::init_list(int /* id */, NeighList *ptr) {
  if (smartlist) {
...
void FixConpOld::setup_post_neighbor(){
  linalg_init();
  post_neighbor();
}

void FixConpOld::setup_pre_force(int vflag){
  force->kspace->setup();
  linalg_setup();
  pre_force(vflag);
}

void FixConpOld::linalg_init()
{
...
void FixConpOld::pre_force(int /* vflag */)
{
...
void FixConpOld::post_force(int vflag) {
  postforceflag = true;
  force_cal(vflag);
}
...
double FixConpOld::compute_scalar()
{
  return addv;
}
...
int FixConpOld::electrode_check(int atomid)
{
...
void FixConpOld::b_comm(double* bsend, double* brecv)
{
...
void FixConpOld::b_bcast(int nproc, int ncount, int* eleall_list, double* bcastbuf)
{
...
void FixConpOld::b_comm_int(int* bsend, int* brecv)
{
...
void FixConpOld::update_bk(bool coulyes, double* bbb_all)
{
  Ktime1 = MPI_Wtime();
  kspmod->b_cal(bbb);
  Ktime2 = MPI_Wtime();
...
/*----------------------------------------------------------------------- */
void FixConpOld::equation_solve()
{
//solve equations
  if (minimizer == 0) {
    cgtime1 = MPI_Wtime();
...
/* ---------------------------------------------------------------------- */
void FixConpOld::force_cal(int vflag)
{
  int i;
...
/*
/* ---------------------------------------------------------------------- */
void FixConpOld::coul_cal(int coulcalflag, double* m)
{
  Ctime1 = MPI_Wtime();
...
/* ---------------------------------------------------------------------- */
void FixConpOld::alist_coul_cal(double* m)
{
  Ctime1 = MPI_Wtime();
  if (alist->occasional) {
    neighbor->build(0);
...
/* ---------------------------------------------------------------------- */
void FixConpOld::blist_coul_cal(double* m)
{
  Ctime1 = MPI_Wtime();
  int i,j,k,ii,jj,jnum,itype,jtype,idx1d;
...
/* ---------------------------------------------------------------------- */
void FixConpOld::blist_coul_cal_post_force()
{
  Ctime1 = MPI_Wtime();
  int i,j,k,ii,jj,jnum,itype,jtype,idx1d;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz;
