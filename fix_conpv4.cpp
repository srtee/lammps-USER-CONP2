...
FixConpV4::FixConpV4(LAMMPS *lmp, int narg, char **arg) :
...
void FixConpV4::request_smartlist() {
...
void FixConpV4::init_list(int /* id */, NeighList *ptr) {
  if (smartlist) {
...
void FixConpV4::setup(int vflag)
{
...
void FixConpV4::pre_force(int /* vflag */)
{
...
void FixConpV4::post_force(int vflag) {
  force_cal(vflag);
}

/* ---------------------------------------------------------------------- */

int FixConpV4::electrode_check(int atomid)
{
...
void FixConpV4::b_comm(double* bsend, double* brecv)
{
...
void FixConpV4::update_bk(bool coulyes, double* bbb_all)
{
  Ktime1 = MPI_Wtime();
  int i,j,k,elei;
  int nmax = atom->nmax;
...
void FixConpV4::sincos_a(int ff_flag,double *eleallz)
{
...
/* ---------------------------------------------------------------------- */
void FixConpV4::force_cal(int vflag)
{
  int i;
...
/*
/* ---------------------------------------------------------------------- */
void FixConpV4::coul_cal(int coulcalflag, double* m)
{
  Ctime1 = MPI_Wtime();
...
/* ---------------------------------------------------------------------- */
void FixConpV4::alist_coul_cal(double* m)
{
  Ctime1 = MPI_Wtime();
  if (alist->occasional) {
    neighbor->build(0);
...
/* ---------------------------------------------------------------------- */
void FixConpV4::blist_coul_cal(double* m)
{
  Ctime1 = MPI_Wtime();
  int i,j,k,ii,jj,jnum,itype,jtype,idx1d;
  int eci,ecj;
...
/* ---------------------------------------------------------------------- */
void FixConpV4::blist_coul_cal_post_force()
{
  Ctime1 = MPI_Wtime();
  int i,j,k,ii,jj,jnum,itype,jtype,idx1d;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz;
...
double FixConpV4::compute_scalar()
{
  return addv;
}

/* ---------------------------------------------------------------------- */
double FixConpV4::rms(int km, double prd, bigint natoms, double q2)
{
